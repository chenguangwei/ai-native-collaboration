...
- Otherwise, before starting Codex, resolve the root and check for a resumable rescue thread:

```bash
_CODEX_ROOT=$(ls -d ~/.claude/plugins/cache/openai-codex/codex/*/ 2>/dev/null | sort -V | tail -1 | tr -d '\n' | sed 's|/$||')
node "$_CODEX_ROOT/scripts/codex-companion.mjs" task-resume-candidate --json
```
...
- The subagent is a thin forwarder only. It should use one `Bash` call to invoke `node "$_CODEX_ROOT/scripts/codex-companion.mjs" task ...` and return that command's stdout as-is.
