...
当脚手架发布新版本时，在**已接入的业务项目**中执行以下步骤：

### 第一步：拉取最新脚手架

```bash
# 拉取脚手架最新代码到 .scaffold/ 目录（--squash 保持历史干净）
git subtree pull --prefix=.scaffold scaffold main --squash
```

### 第二步：同步 AI 配置文件

`.scaffold/` 是只读的原始文件，业务使用的是根目录的副本，需手动同步有变更的部分：

```bash
# 同步核心规则（如有更新）
cp .scaffold/CLAUDE.md ./CLAUDE.md

# 同步命令、规则、Agent（覆盖，注意保留项目自定义内容）
cp -r .scaffold/.claude/commands ./.claude/commands
cp -r .scaffold/.claude/rules ./.claude/rules
cp -r .scaffold/.claude/agents ./.claude/agents

# 同步 Skills（全量覆盖，项目自定义 skill 请勿放在此目录）
cp -r .scaffold/.agents ./.agents
cp -r .scaffold/.claude/skills ./.claude/skills
```

> **注意**：如果你在 `.claude/` 下有项目级自定义内容（如 `project-config.json`、自定义 skill），同步前先备份，合并后恢复。
...
```bash
git add .scaffold/ .claude/ .agents/ CLAUDE.md
git commit -m "chore: 更新 AI Native 脚手架至 v$(cat .scaffold/SCAFFOLD_VERSION)"
```

### 版本查看

```bash
cat .scaffold/SCAFFOLD_VERSION        # 脚手架版本
cat .scaffold/CHANGELOG.md            # 查看更新内容
cat .claude/skills/gstack/VERSION     # gstack 版本
```
