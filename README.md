...
当脚手架发布新版本时，打开业务项目，启动 Claude Code，把下面这段话发给它：

```
请帮我更新 AI Native 脚手架到最新版本。

脚手架已通过 git subtree 接入到 .scaffold/ 目录，remote 名称是 scaffold。

请帮我：
1. 执行 git subtree pull 拉取脚手架最新代码
2. 对比 .scaffold/ 与项目根目录的差异，同步有变更的 .claude/、.agents/、CLAUDE.md
3. 注意保留 .claude/project-config.json 和项目自定义的 skill（不要覆盖）
4. 如果 gstack 版本有更新，重新执行 ./setup 编译二进制
5. 告诉我本次更新了哪些内容，然后提交

注意：不要改动现有 src/ 业务代码。
```

CC 会逐步执行并在每个关键操作前说明做什么，你确认后它继续。
