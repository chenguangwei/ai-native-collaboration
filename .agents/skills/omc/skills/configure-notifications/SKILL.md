...
This is an interactive, natural-language configuration skill. Walk the user through setup by asking questions with AskUserQuestion. Write the result to `~/.Codex/.omc-config.json`.
...
Config saved to: ~/.Codex/.omc-config.json

You can also set these via environment variables:
  OMC_TELEGRAM_BOT_TOKEN=123456789:ABCdefGHI...
  OMC_TELEGRAM_CHAT_ID=123456789

To reconfigure: /oh-my-Codex:configure-notifications telegram
To configure Discord: /oh-my-Codex:configure-notifications discord
To configure Slack: /oh-my-Codex:configure-notifications slack
```
...
To reconfigure: /oh-my-Codex:configure-notifications discord
To configure Telegram: /oh-my-Codex:configure-notifications telegram
To configure Slack: /oh-my-Codex:configure-notifications slack
```
...
To reconfigure: /oh-my-Codex:configure-notifications slack
To configure Discord: /oh-my-Codex:configure-notifications discord
To configure Telegram: /oh-my-Codex:configure-notifications telegram
```
...
Check if `~/.Codex/omc_config.hook.json` exists. If it does, show the current configuration. If not, explain what it does.
...
Config file: ~/.Codex/omc_config.hook.json
```
...
**Options:**
1. **session-end** - When a Codex session finishes (most common)
2. **ask-user-question** - When Codex is waiting for input
3. **session-idle** - When Codex finishes and waits for input
...
Read or create `~/.Codex/omc_config.hook.json` and merge the new settings:
...
- `/oh-my-Codex:configure-openclaw` — Configure OpenClaw gateway integration
...
If `~/.Codex/omc_config.openclaw.json` exists, detect and offer migration:

**Step 1: Detect Legacy Config**
```bash
LEGACY_CONFIG="$HOME/.Codex/omc_config.openclaw.json"
...
| `{{tmuxSession}}` | tmux session name | `Codex:my-project` |
