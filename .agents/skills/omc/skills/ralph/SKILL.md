...
7. **Reviewer verification** (tiered, against acceptance criteria):
   ...
   - If `--critic=critic`, use the Codex `critic` agent for the approval pass
...
7.5 **Mandatory Deslop Pass**:
   - Unless `{{PROMPT}}` contains `--no-deslop`, run `oh-my-Codex:ai-slop-cleaner` in standard mode (not `--review`) on the files changed during the current Ralph session only.
...
8. **On approval**: After Step 7.6 passes (with Step 7.5 completed, or skipped via `--no-deslop`), run `/oh-my-Codex:cancel` to cleanly exit and clean up all state files
...
<Tool_Usage>
- Use `Task(subagent_type="oh-my-Codex:architect", ...)` for architect verification cross-checks when changes are security-sensitive, architectural, or involve complex multi-system integration
- Use `Task(subagent_type="oh-my-Codex:critic", ...)` when `--critic=critic`
...
```
Task(subagent_type="oh-my-Codex:executor", model="haiku", prompt="Add type export for UserConfig")
Task(subagent_type="oh-my-Codex:executor", model="sonnet", prompt="Implement the caching layer for API responses")
Task(subagent_type="oh-my-Codex:executor", model="opus", prompt="Refactor auth module to support OAuth2 flow")
```
...
- Stop when the user says "stop", "cancel", or "abort" -- run `/oh-my-Codex:cancel`
...
- [ ] `/oh-my-Codex:cancel` run for clean state cleanup
</Final_Checklist>
