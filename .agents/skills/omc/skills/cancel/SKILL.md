...
`/oh-my-Codex:cancel` follows the session-aware state contract:
...
```
/oh-my-Codex:cancel --force
```

```
/oh-my-Codex:cancel --all
```
...
4. Team artifacts (`~/.Codex/teams/*/`, `~/.Codex/tasks/*/`, `.omc/state/team-state.json`) are best-effort cleared as part of the legacy fallback.
...
3. If a `session_id` was supplied to `/oh-my-Codex:cancel`, skip legacy fallback entirely and operate solely within that session path; otherwise, consult legacy files in `.omc/state/*.json` only if the state tools report no active session. Swarm remains a shared SQLite/marker mode outside session scoping.
...
#### If Team Active (Codex native)

Teams are detected by checking for config files in `~/.Codex/teams/`:

```bash
# Check for active teams
TEAM_CONFIGS=$(find ~/.Codex/teams -name config.json -maxdepth 2 2>/dev/null)
```
...
```
For each team found in ~/.Codex/teams/:
...
**TeamDelete + Cleanup:**
```
  1. Call TeamDelete() — removes ~/.Codex/teams/{name}/ and ~/.Codex/tasks/{name}/
...
Team "{team_name}" cancelled:
  ...
  - Manual cleanup needed: yes/no
    Path: ~/.Codex/teams/{name}/ and ~/.Codex/tasks/{name}/
```

**Implementation note:** The cancel skill is executed by the LLM, not as a bash script. When you detect an active team:
1. Read `~/.Codex/teams/*/config.json` to find active teams
...
5. Team artifacts (`~/.Codex/teams/*/`, `~/.Codex/tasks/*/`, `.omc/state/team-state.json`) remain best-effort cleanup items invoked during the legacy/global pass.
...
|------|-----------------|----------------|
| Autopilot | Yes (phase, files, spec, plan, verdicts) | `/oh-my-Codex:autopilot` |
| Ralph | No | N/A |
| Ultrawork | No | N/A |
| UltraQA | No | N/A |
| Swarm | No | N/A |
...
- **Team-aware**: Detects native Codex teams and performs graceful shutdown
