---
name: team
description: N coordinated agents on shared task list using Codex native teams
aliases: []
level: 4
---
...
Spawn N coordinated agents working on a shared task list using Codex's native team tools. Replaces the legacy `/swarm` skill (SQLite-based) with built-in team management, inter-agent messaging, and task dependencies -- no external dependencies required.
...
```
/oh-my-Codex:team N:agent-type "task description"
/oh-my-Codex:team "task description"
/oh-my-Codex:team ralph "task description"
```
...
**Storage layout (managed by Codex):**
```
~/.Codex/
  teams/fix-ts-errors/
...
2. **Specialist agents complement executor agents.** Route analysis/review to architect/critic Codex agents and UI work to designer agents. Tmux CLI workers are one-shot and don't participate in team communication.
...
- **Cancel:** `/oh-my-Codex:cancel` requests teammate shutdown, waits for responses (best effort), marks phase `cancelled` with `active=false`, captures cancellation metadata, then deletes team resources and clears/preserves Team state per policy. Handoff files in `.omc/handoffs/` are preserved for potential resume.
...
{
  "team_name": "fix-ts-errors",
  "team_file_path": "~/.Codex/teams/fix-ts-errors/config.json",
...
The team skill supports **hybrid execution** combining Codex agent teammates with external CLI workers (Codex CLI and Gemini CLI). Both types can make code changes -- they differ in capabilities and cost. These are standalone CLI tools, not MCP servers.
...
|---------------|----------|-------------|
| `claude_worker` | Codex agent | Full Codex tool access (Read/Write/Edit/Bash/Task). Best for tasks needing Codex's reasoning + iterative tool use. |
...
**Key difference from Codex teammates:**
- CLI workers operate via tmux, not Codex's tool system
...
|-----------|-----------|-----|
| Iterative multi-step work | Codex teammate | Needs tool-mediated iteration + team communication |
| Code review / security audit | CLI worker or specialist agent | Autonomous execution, good at structured analysis |
| Architecture analysis / planning | architect Codex agent | Strong analytical reasoning with codebase access |
| Refactoring (well-scoped) | CLI worker or executor agent | Autonomous execution, good at structured transforms |
| UI/frontend implementation | designer Codex agent | Design expertise, framework idioms |
| Large-scale documentation | writer Codex agent | Writing expertise + large context for consistency |
| Build/test iteration loops | Codex teammate | Needs Bash tool + iterative fix cycles |
| Tasks needing team coordination | Codex teammate | Needs SendMessage for status updates |
...
The lead runs #1 (Codex security analysis), then #2 and #3 in parallel (Codex refactors backend, designer agent redesigns frontend), then #4 (Codex teammate handles test iteration), then #5 (Gemini final review).

### Pre-flight Analysis (Optional)

For large ambiguous tasks, run analysis before team creation:

1. Spawn `Task(subagent_type="oh-my-Codex:planner", ...)` with task description + codebase context
...
This approach complements the existing `SendMessage`-based communication by providing a pull-based mechanism for MCP workers that cannot use Codex's team messaging tools.
...
1. User invokes `/team ralph "task"` or `/oh-my-Codex:team ralph "task"`
...
4. If architect approves: both modes complete, run `/oh-my-Codex:cancel`
...
1. Check `~/.Codex/teams/` for teams matching the task slug
...
|--------|--------------|----------------------|
| **Storage** | JSON files in `~/.Codex/teams/` and `~/.Codex/tasks/` | SQLite in `.omc/state/swarm.db` |
...
| **Heartbeat** | Automatic idle notifications from Codex | Manual heartbeat table + polling |
...
**When to use Team over Swarm:** Always prefer `/team` for new work. It uses Codex's built-in infrastructure, requires no external dependencies, supports inter-agent communication, and has task dependency management.

## Cancellation

The `/oh-my-Codex:cancel` skill handles team cleanup:
...
If teammates are unresponsive, `TeamDelete` may fail. In that case, the cancel skill should wait briefly and retry, or inform the user to manually clean up `~/.Codex/teams/{team_name}/` and `~/.Codex/tasks/{team_name}/`.
...
> **Note:** Team members do not have a hardcoded model default. Each teammate is a separate Codex session that inherits the user's configured model. Since teammates can spawn their own subagents, the session model acts as the orchestration layer while subagents can use any model tier.

## State Cleanup

On successful completion:

1. `TeamDelete` handles all Codex state:
   - Removes `~/.Codex/teams/{team_name}/` (config)
   - Removes `~/.Codex/tasks/{team_name}/` (all task files + lock)
2. OMC state cleanup via MCP tools:
   ...
   ```
3. Or run `/oh-my-Codex:cancel` which handles all cleanup automatically.
