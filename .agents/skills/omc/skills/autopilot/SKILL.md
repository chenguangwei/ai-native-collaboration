...
- Cancel with `/oh-my-Codex:cancel` at any time; progress is preserved for resume
</Execution_Policy>
...
6. **Phase 5 - Cleanup**: Delete all state files on successful completion
   ...
   - Run `/oh-my-Codex:cancel` for clean exit
</Steps>

<Tool_Usage>
- Use `Task(subagent_type="oh-my-Codex:architect", ...)` for Phase 4 architecture validation
- Use `Task(subagent_type="oh-my-Codex:security-reviewer", ...)` for Phase 4 security review
- Use `Task(subagent_type="oh-my-Codex:code-reviewer", ...)` for Phase 4 quality review
- Agents form their own analysis first, then spawn Codex Task agents for cross-validation
